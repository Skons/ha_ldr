...
parser.add_argument('-m', '--max-value', action='store', dest='max', type=int, help="Max brightness value reported", default=3000)
...
parser.add_argument('-s', '--sleep', action='store', dest='sleep', type=int, help="Number of seconds to wait for next brightness retreival", default=10)
...
if args.logfile is not None:
    logging.basicConfig(filename=args.logfile,filemode='a',format='%(asctime)s %(name)s %(levelname)s %(message)s',datefmt='%Y-%m-%d %H:%M:%S',level=loglevel)
else:
